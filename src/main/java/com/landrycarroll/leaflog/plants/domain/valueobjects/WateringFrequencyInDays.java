...
public record WateringFrequencyInDays(int value) {
    ...
    public WateringFrequencyInDays {
        if (isNegative(value)) {
            throw new DomainValidationException("Value must be greater than or equal to 0");
        }

        if (isExceedsMaximum(value)) {
            throw new DomainValidationException("Value must be less than or equal to " + MAXIMUM_VALUE);
        }
    }
